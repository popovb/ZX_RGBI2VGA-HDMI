...
bool zxrgb::SettingsLoader::inv_pin_mask(const char* s, int v) const {
     if (! check(s + 1, "cap_in_inv_mask") ) return false;
     if (! check(s[0]) ) return false;
     set(cs.inv_pin_mask, s, v);
     return true;
}

bool zxrgb::SettingsLoader::len_vs(const char* s, int v) const {
     if (! check(s + 1, "cap_len_VS") ) return false;
     if (! check(s[0]) ) return false;
     set(cs.len_vs, s, v);
     return true;
}
...
bool zxrgb::SettingsLoader::wide(const char* s, int v) const {
     if (! check(s + 1, "wide_mode") ) return false;
     if (! check(s[0]) ) return false;
     set(cs.wide, s, v);
     return true;
}
